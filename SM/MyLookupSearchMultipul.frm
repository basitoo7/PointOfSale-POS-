...
Begin VB.Form MyLookupOLDBsearchmultipul 
   Caption         =   "Look up :"
   ClientHeight    =   6135
   ClientLeft      =   60
   ClientTop       =   345
   ...
   EndProperty
   Icon            =   "MyLookupSearchMultipul.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   Moveable        =   0   'False
   ...
   End
End
Attribute VB_Name = "MyLookupOLDBsearchmultipul"
...
If SeekText.Text <> "" Then
   If Gs_DirectSearch = 1 Then
     Adodc1.RecordSource = Gs_SQL & IIf(Len(Gs_OtherPara) > 0, Gs_OtherPara & " And " & UCase(Gs_FindFld) & " = '" & UCase(SeekText) & "'" + " " + Gs_OrderBy, " Where " & UCase(Gs_FindFld) & " = '" & UCase(SeekText) & "'" + " " + Gs_OrderBy)
   Else
...
If txtsearchby.Text = "GRNCode" Then
Gs_DirectSearch = 1
Else
Gs_DirectSearch = 0
